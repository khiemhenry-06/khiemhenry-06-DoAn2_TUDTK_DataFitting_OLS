...
\begin{lstlisting}[caption={Trích đoạn tính nghiệm OLS trong \texttt{part1/ols\_implementation.py}}]
...
\subsection{Cài đặt và phân tích Hồi quy Ridge (Ridge Regression)}
\begin{itemize}
    \item \textbf{Khái quát:} Khi dữ liệu đầu vào chứa các đặc trưng có độ tương quan cao (đa cộng tuyến), ma trận $X^TX$ sẽ tiến gần đến trạng thái suy biến, khiến ước lượng OLS thông thường có phương sai rất lớn. Hồi quy Ridge (chuẩn hóa $L_2$) giải quyết vấn đề này bằng cách thêm một thành phần điều chỉnh (penalty term) vào hàm mất mát.

    \item \textbf{Cơ sở toán học:} Nghiệm của Ridge Regression được xác định qua công thức đại số tuyến tính:
\end{itemize}
...
\noindent Trong đó, $\lambda$ (hay alpha) là siêu tham số kiểm soát mức độ phạt, và $I$ là ma trận đơn vị. Lưu ý trong quá trình cài đặt thực tế, hệ số chặn (intercept) không bị phạt, do đó phần tử đầu tiên $I_{00}$ được gán bằng 0.

\begin{lstlisting}[caption={Hàm Ridge Regression cài đặt trong \texttt{part1/ridge\_lasso.py}}]
...
\begin{itemize}
    \item \textbf{Minh họa bằng Ridge Trace:} Nhóm tiến hành vẽ biểu đồ Ridge Trace trên dữ liệu giả lập để quan sát sự co rút của các hệ số hồi quy $\hat{\beta}$. Khi $\lambda$ tăng lên, các hệ số có xu hướng hội tụ dần về 0, giúp giảm phương sai của mô hình và khắc phục hiệu quả hiện tượng đa cộng tuyến.
\end{itemize}
...
\subsection{Cài đặt K-Fold Cross Validation}
\begin{itemize}
    \item \textbf{Khái quát:} K-Fold Cross Validation là kỹ thuật đánh giá chéo giúp ước lượng hiệu suất của mô hình một cách khách quan, hạn chế tối đa tình trạng học vẹt (overfitting).

    \item \textbf{Luồng hoạt động:} Tập dữ liệu được chia ngẫu nhiên thành $K$ phần (fold) có kích thước xấp xỉ nhau. Quá trình huấn luyện sẽ lặp lại $K$ lần. Ở mỗi lần lặp, $K-1$ phần được gộp lại để làm tập huấn luyện (Train set), và phần còn lại đóng vai trò tập kiểm thử (Test set).

    \item \textbf{Đánh giá sai số:} Sai số bình phương trung bình (MSE) được tính toán trên từng tập kiểm thử. Lỗi Cross-Validation cuối cùng là trung bình cộng của các sai số này:
\end{itemize}
...
\begin{itemize}
    \item \textbf{Vận dụng thực tế:} Kỹ thuật này được nhóm cài đặt hoàn toàn thủ công bằng thư viện numpy (không sử dụng hàm có sẵn của sklearn) nhằm phục vụ cho bước dò tìm siêu tham số $\lambda$ tối ưu nhất cho mô hình Ridge/Lasso ở Phần 2.
\end{itemize}

\begin{lstlisting}[caption={Hàm chia K-Fold cài đặt trong \texttt{part1/cross\_validation.py}}]
...
\subsection{Phân tích các thuộc tính trong bộ dữ liệu}
...
\begin{table}[H]
\centering
\caption{Tóm tắt vai trò và cách xử lý các thuộc tính chính}
{\footnotesize
\renewcommand{\arraystretch}{1.32}
\setlength{\tabcolsep}{5pt}
\begin{tabular}{p{3.6cm}p{5.9cm}p{4.9cm}}
\rowcolor{reportblue}
\tableheader{Nhóm thuộc tính} & \tableheader{Ý nghĩa trong dữ liệu} & \tableheader{Quyết định xử lý} \\
\midrule
\rowcolor{gray!10}
\multicolumn{3}{l}{\textbf{Nhóm định danh và thời gian}} \\
\texttt{Name} & Tên trò chơi, chủ yếu dùng để nhận diện từng quan sát. & Không dùng trực tiếp trong mô hình vì không mang dạng số/categorical ổn định. \\
\texttt{Year\_of\_Release} & Năm phát hành, có thể phản ánh xu hướng thị trường theo thời gian. & Không dùng trong pipeline chính do có missing value và cần xử lý thời gian riêng. \\
\midrule
\rowcolor{gray!10}
\multicolumn{3}{l}{\textbf{Nhóm biến phân loại dùng làm đặc trưng}} \\
\texttt{Platform} & Nền tảng phát hành game như PS4, X360, PC, Wii. & Điền \texttt{Unknown}, sau đó one-hot encoding. \\
\texttt{Genre} & Thể loại game, ví dụ Action, Sports, Shooter. & Điền \texttt{Unknown}, sau đó one-hot encoding. \\
\texttt{Publisher} & Nhà phát hành game, có số lượng nhãn lớn. & Giữ top publisher phổ biến, nhóm còn lại thành \texttt{Other}. \\
\midrule
\rowcolor{gray!10}
\multicolumn{3}{l}{\textbf{Nhóm điểm số và lượt đánh giá}} \\
\texttt{Critic\_Score} & Điểm đánh giá trung bình từ chuyên gia. & Chuyển numeric, điền median, chuẩn hóa z-score. \\
\texttt{Critic\_Count} & Số lượt đánh giá từ chuyên gia. & Điền median, chuẩn hóa z-score. \\
\texttt{User\_Score} & Điểm đánh giá trung bình từ người dùng; có giá trị \texttt{tbd}. & Chuyển \texttt{tbd} thành missing, điền median, chuẩn hóa z-score. \\
\texttt{User\_Count} & Số lượt đánh giá từ người dùng. & Điền median, chuẩn hóa z-score. \\
\midrule
\rowcolor{gray!10}
\multicolumn{3}{l}{\textbf{Nhóm biến mô tả bổ sung}} \\
\texttt{Developer}, \texttt{Rating} & Nhà phát triển và phân loại độ tuổi của game. & Không dùng trong pipeline chính để giữ mô hình gọn và ổn định. \\
\midrule
\rowcolor{gray!10}
\multicolumn{3}{l}{\textbf{Nhóm doanh thu}} \\
\texttt{NA\_Sales}, \texttt{EU\_Sales}, \texttt{JP\_Sales}, \texttt{Other\_Sales} & Doanh thu theo khu vực; là thành phần cấu thành \texttt{Global\_Sales}. & Loại bỏ khỏi ma trận đặc trưng để tránh data leakage. \\
\rowcolor{reportgreen!12}
\texttt{Global\_Sales} & Tổng doanh thu toàn cầu. & Biến mục tiêu $y$ của bài toán hồi quy. \\
\bottomrule
\end{tabular}
}
\end{table}

Từ bảng trên, nhóm chỉ giữ các thuộc tính mô tả đặc điểm game trước hoặc tại thời điểm đánh giá như \texttt{Platform}, \texttt{Genre}, \texttt{Publisher}, \texttt{Critic\_Score}, \texttt{Critic\_Count}, \texttt{User\_Score} và \texttt{User\_Count}. Các cột doanh thu khu vực bị loại bỏ vì chúng là thành phần cấu thành trực tiếp của \texttt{Global\_Sales}, nếu đưa vào mô hình sẽ làm sai mục tiêu dự báo.
...
\begin{lstlisting}[caption={Trích đoạn tiền xử lý dữ liệu trong \texttt{part2/data\_pipeline.py}}]
...
\begin{lstlisting}[caption={Huấn luyện OLS Full trong \texttt{part2/model\_comparison.py}}]
full_model = sm.OLS(y_train, X_train_sm).fit()
print(full_model.summary())
\end{lstlisting}

\begin{lstlisting}[caption={Chọn biến và huấn luyện RidgeCV trong \texttt{part2/model\_comparison.py}}]
...
\begin{lstlisting}[caption={Các hàm kiểm chứng phần dư trong \texttt{part1/residual\_analysis.py}}]
...
\begin{lstlisting}[caption={Lớp Kernel Ridge Regression trong \texttt{part2/advanced\_methods.py}}]
class KernelRidgeRegression:
    ...
    def fit(self, X, y):
        ...
        n = len(self.y_train)
        K = self._rbf_matrix(self.X_train, self.X_train)
